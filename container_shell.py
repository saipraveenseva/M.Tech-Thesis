...
# /opt/stack/python-zunclient/v1/containers_shell.py
@utils.arg('url',
           metavar='<url>',
           help='target URL')
@utils.arg('name',
           ...
           help='Name of the container')
def do_pullimage(cs, args):
    r = requests.get(args.url, verify=False,stream=True)
    r.raw.decode_content = True
    with open("filename.tar", 'wb') as f:
        shutil.copyfileobj(r.raw, f)

    call(['openstack', 'image', 'create', '--public', '--container-format', 'docker', '--disk-format', 'raw', '--file', "filename.tar", args.name])
...
@utils.arg('name',
           ...
           help='Name of the container')
@utils.arg('pnum',
           metavar='<process number>',
           help='process number')
def do_proccheckpoint(cs, args):
    call(['zun', 'exec', '-i', args.name, 'criu', 'dump', '-vvvv', '-t', args.pnum, '--shell-job'])


@utils.arg('name',
           metavar='<container_name>',
           help='Name of the container')
def do_procrestore(cs, args):
    call(['zun', 'exec', '-i', args.name, 'criu', 'restore', '-vvvv', '--shell-job'])
